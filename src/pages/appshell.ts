...
export function appShell(session: Session, opts: { title: string; active: Tab; content: string }): Response {
	const mainNav: Tab[] = ["dashboard", "files", "photos", "passwords", "telegram"];
	const bottomNav: Tab[] = ["shared", "activity", "favorites", "trash", "settings"];
	...
	return layout({
		...
		<style>
			:root { --sbw: 248px; }
			.appgrid {
				display:grid; grid-template-columns:var(--sbw) 1fr;
				gap:1.25rem; align-items:start; min-height:calc(100vh - 64px);
			}
			...
			.sb-head {
				padding:1rem 1rem .75rem; border-bottom:1px solid #1E1E2E;
			...
			.slink {
				...
				text-decoration:none; color:#7A7A92; border:1px solid transparent;
			...
			/* Telegram link gets a distinct teal accent when active */
			a.slink[href="/telegram"].active { background:rgba(0,136,204,.12); border-color:rgba(0,136,204,.3); }
			a.slink[href="/telegram"].active .slink-icon { background:rgba(0,136,204,.18); border-color:rgba(0,136,204,.35); color:#5AC8FA; }
			...
			.sb-user {
				...
				background:rgba(15,15,19,.5); border:1px solid #1E1E2E; margin:.5rem;
			}
			.sb-avatar {
			...
			.kbd { font-family:ui-monospace,monospace; font-size:.72rem; padding:.1rem .38rem; border-radius:.4rem; border:1px solid #2A2A3C; background:rgba(255,255,255,.03); color:#4A4A62; flex-shrink:0; }

			/* Clear button for search */
			#clearSearch { background:transparent; border:none; cursor:pointer; color:#4A4A62; font-size:.85rem; padding:.1rem; display:none; }
			#clearSearch.visible { display:block; }
			#clearSearch:hover { color:#A7A7BD; }
			...
			<section class="maincol">
				<div class="topbar">
					<div class="search-box">
						...
						<input id="globalSearch" placeholder="Search files, photos…" autocomplete="off" />
		...
		(function() {
			...
			searchEl.addEventListener('input', () => {
				...
				kbdHint.style.display = q.length ? 'none' : '';
